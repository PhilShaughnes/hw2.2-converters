...
class ConverterTest < MiniTest::Test
  def test_celcius_to_fahrenheit
    assert Converter.new(-30).celcius_to_fahrenheit == -22
    assert conv(0).celcius_to_fahrenheit == 32
    assert conv(-22).celcius_to_fahrenheit == -7.6
    assert conv(22).celcius_to_fahrenheit == 71.6
  end

  def test_fahrenheit_to_celcius
    asserttest(:fahrenheit_to_celcius, [[23, -5],
                                        [33, 0.5556],
                                        [0, -17.7778]])
  end
